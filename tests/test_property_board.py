...
def test_yaml_fixture_composes_separate_profiles_and_shared_public_facts() -> None:
    ...
    assert (loaded.config.starting_cash, loaded.config.movement, loaded.config.turn_limit) == (12, 1, 16)
...
def test_profiles_make_distinct_recorded_buy_decisions_from_the_same_landing() -> None:
    ...
    simulation = build_game(loaded, answers)
...
def test_resolver_collects_rent_and_ends_immediately_when_it_cannot_be_paid() -> None:
    ...
    world = BoardWorld((PlayerState("collector", 10), PlayerState("conserver", 12)), owners)

    after_rent, rent_outcome = resolver.resolve(world, "conserver", None)
    unable_world = BoardWorld((PlayerState("collector", 10), PlayerState("conserver", 0)), owners)
...
def test_composed_game_stops_after_sixteen_turns() -> None:
    loaded = load_game(SCENARIO)
    answers = Answers(
        [{loaded.profiles[actor_id].questions[0]: False} for _ in range(8) for actor_id in ("collector", "conserver")]
    )
    simulation = build_game(loaded, answers)
